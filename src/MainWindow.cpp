...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        const int PAD = 10;
        double sc = std::min((width()-2*PAD)/(double)totalW,
                             (height()-2*PAD)/(double)totalH);

        for (auto &m : m_monitors) {
            int rx = PAD + (int)((m.x - minX) * sc);
            int ry = PAD + (int)((m.y - minY) * sc);
            int rw = std::max(4, (int)(m.width  * sc));
            int rh = std::max(4, (int)(m.height * sc));
            QRect rect(rx, ry, rw, rh);
            bool sel = (m.name == m_selected);

            if (m_wallpapers.contains(m.name) && !m_wallpapers[m.name].isEmpty()
                && !WallpaperApplier::isVideoFile(m_wallpapers[m.name])) {
                QPixmap px(m_wallpapers[m.name]);
                if (!px.isNull())
                    p.drawPixmap(rect, px.scaled(rect.size(),
                        Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation));
                else
                    p.fillRect(rect, QColor(40, 40, 60));
            } else {
                p.fillRect(rect, QColor(40, 40, 60));
            }

            p.setPen(QPen(sel ? QColor(0, 180, 255) : QColor(120, 120, 120), sel ? 3 : 1));
            p.drawRect(rect);
            p.setPen(Qt::white);
            QFont f = p.font(); f.setPointSize(8); p.setFont(f);
            p.drawText(rect, Qt::AlignCenter, m.name);
        }
    }
...
void MainWindow::buildUi()
{
    ...
    m_bindHint->setStyleSheet("color: #888; font-size: 11px;");
    m_bindHint->hide();
    ...
    m_fillCombo->addItems({"Заполнить", "Вписать", "Растянуть", "По центру",
                           "Плиткой", "Верх-лево", "Верх-право",
                           "Низ-лево", "Низ-право",
    ...
    m_rotCombo->addItems({"Альбомный (0\u00b0)", "Книжный (90\u00b0)",
                          "Альбомный перевёрнутый (180\u00b0)",
                          "Книжный перевёрнутый (270\u00b0)"});
...
void MainWindow::populateSettings(const QString &monitorName)
{
    ...
    if (it != m_monitors.cend()) {
        QString orient = (it->width >= it->height) ? "Альбомный" : "Книжный";
        m_orientationLabel->setText(
            QString("Ориентация: %1  |  %2\u00d7%3  @  %4Hz  scale %5")
            .arg(orient).arg(it->width).arg(it->height)
            .arg(it->refreshRate).arg(it->scale));
    }
    ...
    if (isVid) {
        m_bindHint->setText(
            QString("\U0001f4a1 Добавьте в hyprland.conf:\n"
                    "bind = , F9, exec, hyprwall --toggle-audio %1").arg(monitorName));
        m_bindHint->show();
    } else {
        m_bindHint->hide();
    }
...
void MainWindow::onBrowseFile()
{
    ...
    if (isVid) {
        m_bindHint->setText(
            QString("\U0001f4a1 Добавьте в hyprland.conf:\n"
                    "bind = , F9, exec, hyprwall --toggle-audio %1").arg(m_currentMonitor));
        m_bindHint->show();
    } else {
        m_bindHint->hide();
    }
...
void MainWindow::onApply()
{
    ...
    if (!WallpaperApplier::apply(cfg))
        QMessageBox::warning(this, "Ошибка",
            "Не удалось применить обои.\nПроверьте, что hyprpaper / mpvpaper установлены.");
}
