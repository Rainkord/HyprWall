...
{
    switch (mode) {
        case FillMode::Cover:   return "";         // default, no prefix
...
QString WallpaperApplier::prepareRotatedImage(const QString &src, WallpaperRotation rot)
{
    ...
    // Use flipped() (Qt6) to avoid deprecated mirrored()
...
// Write hyprpaper.conf with preload + wallpaper entries for all monitors
static void writeHyprpaperConf(const QMap<QString, WallpaperConfig> &all)
{
    ...
    ts << "\n";
    // Then wallpaper entries with fit_mode prefix
    for (auto it = all.cbegin(); it != all.cend(); ++it) {
        const WallpaperConfig &c = it.value();
        if (c.filePath.isEmpty() || WallpaperApplier::isVideoFile(c.filePath)) continue;
        QString path = c.filePath;
        QString rot = WallpaperApplier::prepareRotatedImage(path, c.rotation);
        if (!rot.isEmpty()) path = rot;
        QString prefix = WallpaperApplier::fillModeToHyprpaper(c.fillMode);
        ts << "wallpaper = " << c.monitorName << "," << prefix << path << "\n";
    }
    f.close();
...
bool WallpaperApplier::apply(const WallpaperConfig &cfg)
{
    ...
    // hyprpaper IPC only supports `wallpaper` with prefix syntax:
    //   "monitor,contain:/path"  or  "monitor,fill:/path"  or  "monitor,/path" (cover)
    // preload via IPC is NOT supported — must restart with config.
    // So we always restart hyprpaper with a fresh config.
